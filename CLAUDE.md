...
- Ollama running locally on the Mac Mini M4, model `llama3.1:8b` (changed from `mistral:7b` in session 6 after a diagnostic model comparison). A second, remote target is available once configured: `gpt-oss:20b` on a trusted, privately administered server reachable over Tailscale — denser, more accurate summaries, but a "thinking" model with its own budget/error handling (SPEC.md §4/§5). Target selection is always a manual switch, never automatic (SPEC.md §2). `num_ctx` is sent per request by `buildGenerationParams` (`8192` local, `10240` remote) — not machine-level config (SPEC.md §5)
...
1. **Frontend-only, no backend.** Ollama is called directly from the browser via its local HTTP API. This requires `OLLAMA_HOST` and `OLLAMA_ORIGINS` configured on the Mac Mini so Ollama accepts non-localhost origins — see SPEC.md §5. Do not introduce a backend/proxy server; the whole point of this project is to stay minimal. The **local** target's Ollama base URL is derived at runtime from `window.location.hostname` — never hardcoded and never a build-time env var — so the same build works whether it's loaded locally or over Tailscale. **Narrow exception (session 6):** the **remote** target's base URL is a build-time env var, `VITE_REMOTE_OLLAMA_HOST` (`.env.local`, gitignored — the repo is public and must never contain the real address), because unlike the Mac Mini it isn't something `window.location.hostname` can resolve. Scoped to the remote target only — the local target's runtime resolution above is unchanged.
...
3. **Full article text is sent to the model, never truncated.** Unlike Newsletter Digest (which truncates to ~1500–2000 characters for short newsletter summaries), Article Brief sends the complete pasted text. This is what the `num_ctx` override exists for — `8192` by default, `10240` for the remote profile (SPEC.md §4/§5).
...
- Documentation sync: `SPEC.md` is the behavioral source of truth. Whenever a requested change adds or alters behavior, a UI element, user-facing copy, or a generation parameter that `SPEC.md` does not already describe, update `SPEC.md` in the same commit as the code: the relevant section, plus a §8 test scenario when applicable. If the change has no mockup in `/design`, say so explicitly in the SPEC entry. Flag every spec addition in the commit summary so it can be reviewed. Never implement an unspecified behavior and leave `SPEC.md` silent about it. Update `CLAUDE.md` only for changes future sessions need to know about (architecture, file structure, conventions, commands, standing rules), not as a per-session log.
...
- File/folder structure:
  ...
  - `/src/lib/prompts.ts` — FR/EN prompt templates. The French prompt is written entirely in French, no English framing mixed in — that, not few-shot examples, is what fixed anglicisms on Newsletter Digest (see SPEC.md §4). Also holds `computeTargetPoints` (feeds only the prompt text, as a soft continuation anchor), `computeNumPredict` (scales the generation budget directly from article word count), and `buildGenerationParams` — the two `compute*` functions are deliberately independent, not chained (SPEC.md §4); kept alongside the prompt templates rather than a separate file since they're the same generation-parameter concern
  - `/src/lib/text-cleanup.ts` — strips standalone URL lines from the copy of the pasted text used for detection/generation (SPEC.md §2); deliberately narrow scope, not general boilerplate stripping
  - `/src/lib/word-count.ts` — shared word-count function: both call sites use the same definition of a word, but not the same input text. PasteArea's and the streaming/done header's displayed counts (SPEC.md §2.2) are computed on the raw pasted text; `computeTargetPoints`/`computeNumPredict` (prompts.ts) are computed on the cleaned text — the one actually sent to the model (SPEC.md §2.13). The two can therefore differ by the number of stripped bare-URL lines, typically one word each
...
- `/design` — validated UI mockups (PNG), exported from Claude Design, one per state and breakpoint: `desktop-01-idle.png`, `desktop-02-filled.png`, `desktop-03-streaming.png`, `desktop-04-done.png`, `mobile-01-idle.png`, `mobile-02-filled.png`, `mobile-03-streaming.png`, `mobile-04-done.png`, plus, added in session 6 for the remote target and the extended error contract: `desktop-05-filled-remote.png`, `desktop-06-error-unreachable.png`, `desktop-07-error-empty.png`, `desktop-08-streaming-remote.png`, `mobile-05-filled-remote.png`, `mobile-06-error-unreachable.png`, `mobile-07-error-empty.png`, `mobile-08-streaming-remote.png`. Source of truth for layout and copy unless a session's transition notes say a screen was revised since export. Exact color/typography values are also recorded in SPEC.md §6 as a convenience reference.
